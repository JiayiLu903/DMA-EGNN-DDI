...
def train_test_data3(data_lis, drugs):
    ...
    global_all_labels = set()
    for item in data_lis:
        label = tuple(item[1]) if isinstance(item[1], (list, np.ndarray)) else item[1]
        global_all_labels.add(label)
    print(f"数据集中共有 {len(global_all_labels)} 个类别: {global_all_labels}")

    for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(drugs)):
        train_drugs = drugs[train_idx]
        test_drugs = drugs[test_idx]
        train_X, train_Y = [], []
        test_X, test_Y = [], []

        if multi_class:

            candidate_test_pairs = []
            for item in data_lis:
                if item[0][0] in test_drugs and item[0][1] in test_drugs:
                    candidate_test_pairs.append(item)


            initial_label_counts = {}
            for item in candidate_test_pairs:
                label = tuple(item[1]) if isinstance(item[1], (list, np.ndarray)) else item[1]
            ...
            train_label_items = {}
            for item in data_lis:
                if item[0][0] in train_drugs or item[0][1] in train_drugs:
                    label = tuple(item[1]) if isinstance(item[1], (list, np.ndarray)) else item[1]
                    if label not in train_label_items:
                        train_label_items[label] = []
                    train_label_items[label].append(item)


            added_samples = []
            for label in global_all_labels:
                current_count = initial_label_counts.get(label, 0)


                needed = max(2 - current_count, 0) if current_count > 0 else 2

                if needed > 0 and label in train_label_items:

                    available = min(needed, len(train_label_items[label]))
                    for i in range(available):
                        added_item = train_label_items[label][i]
                        added_samples.append(added_item)
                        print(f"Fold {fold_idx + 1}: 添加 {label} 类别的样本 {added_item[0]}")


            for item in added_samples:
                test_X.append(item[0])
                test_Y.append(*item[1:])


            added_pairs = set(tuple(item[0]) for item in added_samples)
            for item in data_lis:
                drug_pair = tuple(item[0])
                if drug_pair[0] in train_drugs and drug_pair[1] in train_drugs:
                    if drug_pair not in added_pairs:
            ...
            final_labels = [tuple(y) if isinstance(y, (list, np.ndarray)) else y for y in test_Y]
            unique_labels = set(final_labels)
            label_counts = {label: final_labels.count(label) for label in unique_labels}

            print(f"Fold {fold_idx + 1} 测试集最终包含 {len(unique_labels)} 个类别")
            ...
            for label in global_all_labels:
                if label_counts.get(label, 0) < 2:
                    print(f"警告: 类别 {label} 只有 {label_counts.get(label, 0)} 个样本")
        else:
        ...
        train_Y_data.append(train_Y)
        test_X_data.append(test_X)
        test_Y_data.append(test_Y)

    return (
        np.array(train_X_data, dtype=object),
        np.array(train_Y_data, dtype=object),
        np.array(test_X_data, dtype=object),
        np.array(test_Y_data, dtype=object)
    )
...
if __name__ == '__main__':
    ...
    if multi_class == True:
        ...
        filename = ['Multi_label_dataset.csv',
                    'drug_KG_features_max_hops_3.csv', 'drug_graph_features.csv',
                    'drug_smiles_simple_embedding_400D.csv', 'drug_smiles.csv']
